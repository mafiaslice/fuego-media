...
export const fetchProjects = (category?: string): Promise<any[]> =>
  fetch(`${BASE}/projects${category ? `?category=${category}` : ""}`).then(r => r.json());

export const fetchBtsList = (): Promise<any[]> =>
  fetch(`${BASE}/bts`).then(r => r.json());

export const fetchBtsEntry = (slug: string): Promise<any> =>
